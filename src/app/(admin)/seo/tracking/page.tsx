...
// Compact metric chip used in the rollup header and grand totals.
function Stat({
  label,
  value,
  tone,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Fix status consolidated per client and per site.
          </p>
        </div>
        <Link
          href="/seo"
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-x-10 gap-y-4 py-5">
          ...
          <Stat
            label="Critical open"
            value={grand.criticalOpen}
            tone="red"
          />
          <Stat
            label="Auto-fixable now"
            value={grand.autoFixableOpen}
            tone="default"
          />
            ...
            <CardHeader>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    {client.totals.sites} site
                    {client.totals.sites === 1 ? "" : "s"}
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  ...
                  {client.totals.autoFixableOpen > 0 && (
                    <span className="inline-flex items-center gap-1 text-primary">
                      <Wrench className="size-3.5" />
                      {client.totals.autoFixableOpen} auto-fixable
                    </span>
                  )}
                ...
                <TableBody>
                  {client.sites.map((site) => (
                    <TableRow key={site.blogId}>
                      ...
                      <TableCell
                        className={`text-right tabular-nums ${scoreTone(site.score)}`}
                      >
                        {site.score ?? "—"}
                      </TableCell>
                      ...
                      <TableCell className="text-right tabular-nums">
                        {site.failed > 0 ? (
                          <span className="text-red-600">{site.failed}</span>
                        ) : (
                          0
                        )}
                      </TableCell>
