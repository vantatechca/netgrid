...
/** Save a client's optional global dosage list (used the next time the matrix
 *  is built). Empty = compound × location pages only. */
export async function updateClientDosages(
  clientId: string,
  dosages: string,
): Promise<{ success: boolean; message: string }> {
  await requireAdmin();
  const clean = dosages.trim();
  await db
    .update(clients)
    .set({ peptideDosages: clean || null, updatedAt: new Date() })
    .where(eq(clients.id, clientId));
  revalidatePath(`/clients/${clientId}`);
  const n = parseDosages(clean).length;
  return { success: true, message: clean ? `Saved ${n} dosage${n === 1 ? "" : "s"}.` : "Dosages cleared." };
}
...
export async function buildLocationMatrix(clientId: string): Promise<{
  ...
  const [client] = await db
    .select({
      ...
      dosages: clients.peptideDosages,
    })
    .from(clients)
    .where(eq(clients.id, clientId))
    .limit(1);
  ...
  // Dosage variants are optional: always build the plain (no-dosage) page, plus
  // one per dosage in the client's global list.
  const dosageVariants = ["", ...parseDosages(client.dosages)];
  ...
  for (const blog of blogRows) {
    ...
    for (const compound of compounds) {
      for (const location of locations) {
        for (const dosage of dosageVariants) {
          rows.push({
            blogId: blog.id,
            clientId,
            compound,
            location,
            dosage,
            title: buildLocationTitle(compound, location, dosage),
          });
        }
      }
    }
...
async function generateLocationTarget(target: LocationTarget): Promise<boolean> {
  const dosage = target.dosage?.trim() || undefined;
  // Buy-phrases hyperlinked in-body to the site's money URL (the funnel).
  const buyTerm = dosage ? `buy ${target.compound} ${dosage}` : `buy ${target.compound}`;
  try {
    const res = await runGenerateAndPublish({
      blogId: target.blogId,
      topic: target.title,
      keywords: buildLocationKeywords(target.compound, target.location, dosage),
      buyLinkTerms: [buyTerm, `buy ${target.compound}`, target.compound],
